    ...
    <div class="card card-custom">
        ...
        <div class="card-header flex-wrap border-0 pt-6 pb-0">
            <div class="card-title">
                <h3 class="card-label">Employee Leave Application Listing</h3>
            </div>
            <div class="card-toolbar">
                <!--begin::Dropdown-->
                <div class="dropdown dropdown-inline mr-2">
                    <button class="btn" data-toggle="modal" data-target="#import">
                     <span class="svg-icon svg-icon-primary svg-icon-2x" data-toggle="tooltip" title="Import">
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form action="#" method="POST" enctype="multipart/form-data">
                                        ...
                                        <br>
                                        <button type="submit" class="btn btn-light-success font-weight-bold mr-2">Import</button>
                                    </form>
                                </div>
                            </div>
                        </div>
                    </div>
                    <a href="#" data-toggle="tooltip" title="Export">
                ...
                @can('Create Leave Application')
                    <a href="{{ route('employee-leave-application.create') }}" class="btn btn-primary font-weight-bolder">
        ...
        <!--begin::Body-->
        <div class="card-body">
            <table class="table" id="employeeLeaveApplicationTable">
                ...
                @foreach($items as $item)
                    <tr>
                        ...
                        <td>
                            @can("Edit Employee Leave Application")
                                @if($item->status !== 1)
                                    <a href="{{ route('employee-leave-application.edit', ['employeeLeaveApplication' => $item->id]) }}"><i class="fa fa-edit" style="color: green"></i></a> ||
                                @endif
                            @endcan
                            @can("Delete Employee Leave Application")
                                <a href="#" onclick="deleteAlert('{{ route('employee-leave-application.delete', ['employeeLeaveApplication' => $item->id]) }}')"><i class="fa fa-trash" style="color: red"></i></a>
                            @endcan
                        </td>
                    </tr>
                @endforeach
...
@section('footer-js')
    <script>
        $(document).ready( function () {
            $('#employeeLeaveApplicationTable').DataTable({
                "order": [],
                "ordering": true,
                "paging": false,
                "bInfo": false,
