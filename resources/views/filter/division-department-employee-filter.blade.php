        ...
        $('#office_division_id').change(function () {
            $.ajax({
                ...
                success: function (res) {
                    ...
                    $('#user_id').append('<option value="0">Select an option</option>');
        ...
        $('#department_id').change(function () {
            $.ajax({
                ...
                success: function (res) {
                    ...
                    var items = '<option value="0">Select an option</option>';
                    $.each(res.data, function (x, y) {
                        items += '<option value="' + y.id + '">' + y.name + '--' + y.fingerprint_no + '</option>';
                    })
                    $('#user_id').append(items)
                },
                error: function (err) {
                    console.log(err)
                }
