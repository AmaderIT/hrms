...
@section('content')
    <div class="card card-custom" id="attendanceReportView">
        <div class="card-body">
            <div class="row mb-4">
                ...
                <div class="col-lg-7 text-right">
                    @can("Export Salary CSV")
                        <form class="form1" action="{{ route('salary.salaryExport', ['salaryDepartment' => $uuid]) }}"
                              method="POST">
                            @csrf
                            <input type="hidden" name="type" value="Export CSV"/>
                            <button title="Export CSV" class="btn btn-sm btn-primary ml-2 salary_btn mb-2"
                                    type="submit"><i class="fa fa-file-csv salary_btn_csv"></i> CSV
                            </button>
                        </form>
                    @endcan
                    @can("Export Salary PDF")
                        <form class="form1" action="{{ route('salary.salaryExport', ['salaryDepartment' => $uuid]) }}"
                            ...
                            <button title="Export PDF" class="btn btn-sm btn-primary ml-2 salary_btn mb-2"
                                    type="submit"><i class="fa fa-file-pdf salary_btn_pdf"></i> PDF
                            </button>
                        </form>
                    @endcan
                    @can("Export Salary Bank Statement CSV")
                        <form class="form1" action="{{ route('salary.salaryExport', ['salaryDepartment' => $uuid]) }}"
                            ...
                            <button title="Bank Statement CSV" class="btn btn-sm btn-primary ml-2 salary_btn mb-2"
                                    type="submit"><i class="fa fa-file-csv salary_btn_csv"></i> Bank Statement
                            </button>
                        </form>
                    @endcan
                    @can("Export Salary Bank Statement PDF")
                        <form class="form1" action="{{ route('salary.salaryExport', ['salaryDepartment' => $uuid]) }}"
                            ...
                            <button title="Bank Statement PDF" class="btn btn-sm btn-primary ml-2 salary_btn mb-2"
                                    type="submit"><i class="fa fa-file-pdf salary_btn_pdf"></i> Bank Statement
                            </button>
                        </form>
                    @endcan
                </div>
            ...
            <div>
                <table class="table table-responsive table-bordered text-center table-condensed salary-table"
                       style="max-height: 800px;overflow: auto;position: relative;">
                    <thead>
                    <tr>
                        <th colspan="3" style="background-color: #e3e3e3;">SALARY FOR THE MONTH OF:</th>
                        <th>{{ strtoupper(date('F', mktime(0, 0, 0, (int)$salaries->first()->month))) }} {{ $salaries->first()->year }}</th>
                    ...
                    <tr style="position: sticky; top: 0;vertical-align: top; z-index: 1">
                        ...
                        <th style="background-color: #e3e3e3;" class="align-middle" rowspan="2">Medical Allowance
                            (Tk.)
                        </th>
                        ...
                        <th style="background-color: #e3e3e3;" class="align-middle" rowspan="1" colspan="4">Attendance
                            (Days)
                        </th>
                        ...
                        <th style="background-color: #e3e3e3;" class="align-middle" rowspan="1" colspan="3">Holiday
                            (Days)
                        </th>
                        ...
                        <th style="background-color: #e3e3e3;" class="align-middle" rowspan="1" colspan="7">Adjustment /
                            Deduction
                        </th>
                    ...
                    @foreach($salaries as $key => $data)
                        ...
                        <tr>
                            ...
                            <td class="net_payable_amount_{{$data->uuid}}">{{ \App\Http\Controllers\SalaryController::currencyFormat($data->net_payable_amount) }}</td>
                            {{--<td>
                                @php
                            ...
                            <td>
                                @if($salaryDepartment->divisional_approval_status === 0 && $salaryDepartment->status == 0 && $salaryDepartment->departmental_approval_status !== 2 && in_array($salaryDepartment->office_division_id, $divisionIds))
                                    @can('Salary Divisional Approval')
                                        <a href="#" data-toggle="modal" data-target="#approvalModal-divisional">Click to
                                            Approve</a>
                                    @endcan
                                @else
                            ...
                            <td>
                                @if($salaryDepartment->departmental_approval_status === 0 && $salaryDepartment->status == 0 && $salaryDepartment->divisional_approval_status !== 2 && in_array($salaryDepartment->department_id, $departmentIds))
                                    @can('Salary Departmental Approval')
                                        <a href="#" data-toggle="modal" data-target="#approvalModal-departmental">Click
                                            to Approve</a>
                                    @endcan
                                @else
                            ...
                            <td>
                                ...
                                @if($salaryDepartment->hr_approval_status === 0 && $salaryDepartment->status == 0 && $hrRejectionCondition)
                                    @can('Salary HR Approval')
                                        <a href="#" data-toggle="modal" data-target="#approvalModal-hr">Click to
                                            Approve</a>
                                    @endcan
                                @else
                            ...
                            <td>
                                @if($salaryDepartment->accounts_approval_status === 0 && $salaryDepartment->status == 0 && $salaryDepartment->hr_approval_status == 1)
                                    @can('Salary Accounts Approval')
                                        <a href="#" data-toggle="modal" data-target="#approvalModal-accounts">Click to
                                            Approve</a>
                                    @endcan
                                @else
                            ...
                            <td>
                                @if($salaryDepartment->managerial_approval_status === 0 && $salaryDepartment->status == 0 && $salaryDepartment->accounts_approval_status == 1 && $salaryDepartment->hr_approval_status == 1)
                                    @can('Salary Managerial Approval')
                                        <a href="#" data-toggle="modal" data-target="#approvalModal-managerial">Click to
                                            Approve</a>
                                    @endcan
                                @else
    ...
    @can("Salary Divisional Approval")
        <div class="modal fade" id="approvalModal-divisional" tabindex="-1" role="dialog"
                            ...
                            <div class="row">
                                <div class="col-lg-12">
                                    ...
                                    <div class="form-group mb-1" id="reason-divisional">
                                        ...
                                        <textarea class="form-control" id="reason" rows="3" name="reject_reason"
    ...
    @can("Salary Departmental Approval")
        <div class="modal fade" id="approvalModal-departmental" tabindex="-1" role="dialog"
             aria-labelledby="exampleModalSizeXl" aria-hidden="true">
            <div class="modal-dialog modal-dialog-scrollable modal-md" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                            ...
                            <div class="row">
                                <div class="col-lg-12">
                                    ...
                                    <div class="form-group mb-1" id="reason-departmental">
                                        ...
                                        <textarea class="form-control" rows="3" name="reject_reason"
                                                  placeholder="Enter Reject Reason"></textarea>
                                    </div>
                                </div>
                            </div>
    ...
    {{-- HR Approval Modal --}}
    @can("Salary HR Approval")
        <div class="modal fade" id="approvalModal-hr" tabindex="-1" role="dialog" aria-labelledby="exampleModalSizeXl"
             aria-hidden="true">
            <div class="modal-dialog modal-dialog-scrollable modal-md" role="document">
                <div class="modal-content">
                    <div class="modal-header">
    ...
    @can("Salary Accounts Approval")
        <div class="modal fade" id="approvalModal-accounts" tabindex="-1" role="dialog"
             aria-labelledby="exampleModalSizeXl" aria-hidden="true">
            <div class="modal-dialog modal-dialog-scrollable modal-md" role="document">
                <div class="modal-content">
                    <div class="modal-header">
    ...
    @can("Salary Managerial Approval")
        <div class="modal fade" id="approvalModal-managerial" tabindex="-1" role="dialog"
             aria-labelledby="exampleModalSizeXl" aria-hidden="true">
            <div class="modal-dialog modal-dialog-scrollable modal-md" role="document">
                <div class="modal-content">
                    <div class="modal-header">
