    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="card card-custom gutter-b example example-compact">
                ...
                <!--begin::Form-->
                <form action="{{ route('user-bonus.generateBonusReportView') }}" method="GET">
                    <div class="card-body">
                        <div class="col-md-8 offset-md-2">
                            {{-- Bonus Name --}}
                            <div class="form-group">
                                <label for="bonus_id">Bonus Name</label>
                                <select class="form-control" id="bonus_id" name="bonus_id" required="required">
                                    <option value="" selected="selected">Select</option>
                                    @foreach($data["bonuses"] as $bonusId => $bonusName)
                                        <option
                                            value="{{ $bonusId }}" {{ $bonusId == old("bonus_id") ? 'selected' : '' }}>
                                            {{ $bonusName }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="department_id[]" id="department_id" multiple>
                                    ...
                                    @foreach($data["officeDepartments"] as $officeDepartment)
                                        <option
                                            value="{{ $officeDepartment->id }}" {{ $officeDepartment->id == old("department_id") ? 'selected' : '' }}>
                            ...
                            <div class="form-group">
                                <label for="year">Report Type<span style="color: red"> *</span></label>
                                @if ( old('filter_type') == null )
                                    <div class="checkbox-group required">
                                        <input value="1" checked type="checkbox" class="filter_type" name="filter_type[]" autocomplete="off"/> Employee
                                        <input value="2" checked type="checkbox" class="filter_type pl-2" name="filter_type[]" autocomplete="off"/> Department
                                    </div>
                                @else
                                    <div class="checkbox-group required">
                                        <input value="1" type="checkbox" {{ in_array(1, old('filter_type') ?? []) ? 'checked' : '' }} class="filter_type" name="filter_type[]" autocomplete="off"/> Employee
                                        <input value="2" type="checkbox" class="filter_type pl-2" {{ in_array(2, old('filter_type') ?? []) ? 'checked' : '' }} name="filter_type[]" autocomplete="off"/> Department
                                    </div>
                                @endif
                                @error("filter_type")
                                <p class="text-danger"> {{ $errors->first("filter_type") }} </p>
                                @enderror
                            </div>
                        </div>
                    </div>
