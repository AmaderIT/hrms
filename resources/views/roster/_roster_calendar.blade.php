...
<div class="card card-custom card-stretch gutter-b">
    <div class="card-header border-0 py-6">
        ...
        <div class="custom-upcoming-holiday">
            <ul class="list-group">
                <div class="list-group">
            <span
                class="list-group-item list-group-item-action flex-column align-items-start active"
                style="text-align: center;color: #e36464;font-size: .85rem;">
                    UPCOMING HOLIDAYS
                </span>
                </div>
                @if(!empty($data["reportToEmployee"]["upcomingHolidays"]) && count($data["reportToEmployee"]["upcomingHolidays"])>0)
                    @foreach($data["reportToEmployee"]["upcomingHolidays"] as $upcomingHoliday)
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            ...
                            <span class="badge badge-primary badge-pill">{{ \Carbon\Carbon::createFromDate($upcomingHoliday->from_date->toDateTimeString())->format('M jS') }}
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        var calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            headerToolbar: {
                left: 'title',
                right: 'prev,next dayGridMonth, listYear',
            },
            views: {
            ...
            dateClick: function (info) {
                ...
                //console.log("asdfasdf ",$(this).find('td.relaxDay'));
                $.ajax({
            ...
            moreLinkContent: function (args) {
                let __countableNumber = args.num;
                let count_element = '';
                let _str = '';
                count_element = document.createElement('span');
                count_element.setAttribute('style', 'font-size: 16px;');
                for (let i = 0; i < __countableNumber; ++i) {
                    _str += '&bull;';
                }
                count_element.innerHTML = _str;
                $('.fc-more-link').removeClass('fc-more-link');
                $('.fc-daygrid-more-link').removeClass('fc-daygrid-more-link');
                //$('.fc-popover.fc-more-popover').hide();
                return {
