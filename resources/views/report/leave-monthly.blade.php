                    ...
                    <div class="card-body">
                        <div class="col-md-8 offset-md-2">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="department_id[]" id="department_id" required multiple>
                                </select>
...
<script type="text/javascript">
    ...
    $('#office_division_id').change(function(){
        ...
        let role = "{{ auth()->user()->hasRole([\App\Models\User::ROLE_ADMIN, \App\Models\User::ROLE_HR_ADMIN_SUPERVISOR]) }}";
        if(role) {

            url = "{{ route('salary.getDepartmentByOfficeDivision', ':officeDivision') }}";
            ...
            url = "{{ route('salary.getSupervisorDepartmentByOfficeDivision', ':officeDivision') }}";
            url = url.replace(":officeDivision", _officeDivisionID);
    ...
    $('#department_id').change(function() {
        ...
        $.post(url, data, function (response, status) {
            ...
            $.each(response.data, function(key, value) {
                $("#user_id").append('<option value="' + value.id + '">'+ value.name + ' - ' + value.fingerprint_no + '</option>');
            });
        })
    });
