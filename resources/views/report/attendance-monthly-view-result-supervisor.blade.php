                    ...
                    <div class="col-12">
                        <h2>{{ $report[0]["department"] . " " . $report[0]["monthAndYear"] }}
                            <div class="float-right">
                                <form action="{{ route('report.generateAttendanceReportPdfToSupervisor') }}" method="GET">
                            ...
                            @foreach($report as $value)
                                <tr>
                                    ...
                                    @foreach($value["report"] as $day => $attendanceData)
                                        @php
                                            $entryTime = reset($attendanceData)["entry"];
                                            $entry = "";
                                            if(isset($entryTime)) $entry = date("h:iA", strtotime($entryTime->punch_time));

                                            $exitTime = reset($attendanceData)["exit"];
                                            $exit = "";
                                            if(isset($exitTime)) $exit = date("h:iA", strtotime($exitTime->punch_time));

                                            $reason = reset($attendanceData)["reason"];
                                        @endphp
                                        <td class="text-center">
                                            {{ $reason }} {{ $entry }} <br/> {{ $exit }}
                                        </td>
                                    @endforeach
                                </tr>
                            @endforeach
