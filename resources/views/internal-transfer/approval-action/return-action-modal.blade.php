<div class="modal-header">
    <h5 class="modal-title">Challan No: <span class="font-weight-400">{{ str_pad($challan->challan, 7, '0', STR_PAD_LEFT) }}</span></h5>
            ...
            <div class="col-md-6">
                <div class="col-md-12" >
                    <div class="form-group mb-1">
                        ...
                        <span style="font-weight: 400">@if(!empty($challan->preparedBy)) {{$challan->preparedBy->name}} @endif</span>
                    </div>
                </div>
                <div class="col-md-12">
                    <div class="form-group mb-1">
                        <label for="challan" style="font-weight: 600 !important;">Authorized By:</label>
                        <span style="font-weight: 400">@if(!empty($challan->authorizedBy)) {{$challan->authorizedBy->name}} @endif</span>
                    </div>
                </div>
                <div class="col-md-12" id="source_warehouse">
                    <div class="form-group mb-1">
                        ...
                        <span style="font-weight: 400">@if(!empty($challan->securityCheckedOutBy)) {{$challan->securityCheckedOutBy->name}} @endif</span>
                    </div>
                </div>
                ...
                @endif
                @if($challan->workflow_type == GENERAL_WORKFLOW)
                    <div class="col-md-12" id="source_warehouse">
                        <div class="form-group mb-1">
                            <label for="source_warehouse_id" style="font-weight: 600 !important;">Security Checked In:</label>
                            <span style="font-weight: 400">@if(!empty($challan->securityCheckedInBy)) {{$challan->securityCheckedInBy->name}} @endif</span>
                        </div>
                    </div>
                    ...
                    <div class="col-md-12" id="destination_warehouse">
                        <div class="form-group mb-1">
                            ...
                            <span style="font-weight: 400">@if(!empty($challan->receivedBy)) {{$challan->receivedBy->name}} @endif</span>
                        </div>
                    </div>
                @endif
                ...
                @if($challan->status == 6)
                    <div class="col-md-12" id="destination_warehouse">
                        <div class="form-group mb-1">
                            ...
                            <span style="font-weight: 400">@if(!empty($challan->rejectedBy)) {{$challan->rejectedBy->name}} @endif</span>
                        </div>
                    </div>
                @endif
            </div>
        ...
        <div class="card">
            <div class="card-body">
                <label style="margin-left: 15px">
                    <input type="radio" name="action" class="action" value="return"/>
                    Return
                </label>
                <br>
                <form class="return" action="{{route('internal-transfer.returnInitiate')}}" method="POST" style="display: none">
                    @csrf
                    <input type="hidden" name="id" value="{{$challan->id}}"><br>
